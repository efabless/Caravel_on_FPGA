...
class SPI:
    def __init__(self, enabled=True):
        ...
        if enabled:
            ...
            self.miso = Pin(4, mode=Pin.IN)  # PF8 = IO[1] = caravel output
            self.cs = Pin(5, mode=Pin.OUT, value=1)
            self.sck = Pin(2, mode=Pin.OUT, value=0)
            self.mosi = Pin(3, mode=Pin.OUT)  # PF9 = IO[2] = caravel input
            ...
            self.cs = Pin(5, mode=Pin.IN, pull=None)
            self.sck = Pin(2, mode=Pin.IN, pull=None)
            self.mosi = Pin(3, mode=Pin.IN, pull=None)  # PF9 = IO[2] = caravel input
            self.miso = Pin(4, mode=Pin.IN, pull=None)  # PF8 = IO[1] = caravel output
            self.spi = None
